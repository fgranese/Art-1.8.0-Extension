...
def FeaturesDataLoaderToNdarray(loader: DataLoader) -> (np.ndarray, np.ndarray):
    features = []
    with torch.no_grad():
        for batch_idx, (data, _) in enumerate(
                tqdm(loader, ascii=True, ncols=100, colour='red', desc='Dataloder to Numpy array')):
            features.append(data.detach().cpu().numpy())
